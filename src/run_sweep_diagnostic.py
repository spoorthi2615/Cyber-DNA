...
def main():
    print("Running diagnostic sweep for Departmental Similarity Filter...")
    ...
    print(f"Total alerts flagged by XGBoost (13-feature Configuration E) before suppression: {total_alerts_before}")
    ...
    for z in z_thresholds:
        ...
        auprc = calculate_auprc(y_test, post_scores)
        
        print(f"Z = {z:+.1f} | Precision: {prec:.4f} | Recall: {rec:.4f} | F1: {f1:.4f} | AUPRC: {auprc:.4f}")
        print(f"          Suppressed: {suppressed_count} (Benign: {benign_suppressed}, Malicious: {malicious_suppressed})")
